...
<div class="card">
    <div class="card-body">
        <form action="{{ route('meetings.participants.update', [$meeting, $participant]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.5rem;">
                <div>
                    ...
                    <select name="participant_type" class="form-select @error('participant_type') is-invalid @enderror">
                        ...
                        <option value="guest" {{ old('participant_type', $participant->participant_type) === 'guest' ? 'selected' : '' }}>Guest</option>
                        <option value="presenter" {{ old('participant_type', $participant->participant_type) === 'presenter' ? 'selected' : '' }}>Presenter</option>
                        <option value="observer" {{ old('participant_type', $participant->participant_type) === 'observer' ? 'selected' : '' }}>Observer</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="attendance_status" class="form-select @error('attendance_status') is-invalid @enderror">
                        ...
                        <option value="present" {{ old('attendance_status', $participant->attendance_status) === 'present' ? 'selected' : '' }}>Present</option>
                        <option value="absent" {{ old('attendance_status', $participant->attendance_status) === 'absent' ? 'selected' : '' }}>Absent</option>
                        <option value="apology" {{ old('attendance_status', $participant->attendance_status) === 'apology' ? 'selected' : '' }}>Apology</option>
                    </select>
