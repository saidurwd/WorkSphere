...
<div class="page-header">
    <div class="page-header-row">
        <div>
            <h1 class="page-title">Edit Obligation</h1>
        ...
        <form action="{{ route('obligations.update', $obligation) }}" method="POST">
            ...
            <div class="grid-2" style="margin-bottom: 1.5rem;">
                <div class="form-group">
                    ...
                    <input type="text" name="title" class="form-input" value="{{ old('title', $obligation->title) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select name="obligation_type_id" class="form-select" required>
                        ...
                        @foreach($types as $type)
                            <option value="{{ $type->id }}" {{ old('obligation_type_id', $obligation->obligation_type_id) == $type->id ? 'selected' : '' }}>
                                {{ $type->type_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="category_id" class="form-select" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $obligation->category_id) == $category->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select name="department_id" class="form-select" required>
                        ...
                        @foreach($departments as $department)
                            <option value="{{ $department->id }}" {{ old('department_id', $obligation->department_id) == $department->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select name="location_id" class="form-select">
                        ...
                        @foreach($locations as $location)
                            <option value="{{ $location->id }}" {{ old('location_id', $obligation->location_id) == $location->id ? 'selected' : '' }}>
            ...
            <div class="grid-2" style="margin-bottom: 1.5rem;">
                <div class="form-group">
                    ...
                    <select name="owner_user_id" class="form-select" required>
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('owner_user_id', $obligation->owner_user_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="backup_user_id" class="form-select">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('backup_user_id', $obligation->backup_user_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid-2" style="margin-bottom: 1.5rem;">
                <div class="form-group">
                    ...
                    <select name="reviewer_user_id" class="form-select">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('reviewer_user_id', $obligation->reviewer_user_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="approver_user_id" class="form-select">
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('approver_user_id', $obligation->approver_user_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid-2" style="margin-bottom: 1.5rem;">
                <div class="form-group">
                    ...
                    <input type="date" name="start_date" class="form-input" value="{{ old('start_date', $obligation->start_date->format('Y-m-d')) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="expiry_date" class="form-input" value="{{ old('expiry_date', $obligation->expiry_date->format('Y-m-d')) }}" required>
            ...
            <div class="grid-2" style="margin-bottom: 1.5rem;">
                <div class="form-group">
                    ...
                    <select name="priority" class="form-select" required>
                        <option value="low" {{ old('priority', $obligation->priority) === 'low' ? 'selected' : '' }}>Low</option>
                        <option value="medium" {{ old('priority', $obligation->priority) === 'medium' ? 'selected' : '' }}>Medium</option>
                        <option value="high" {{ old('priority', $obligation->priority) === 'high' ? 'selected' : '' }}>High</option>
                        <option value="critical" {{ old('priority', $obligation->priority) === 'critical' ? 'selected' : '' }}>Critical</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="risk_level" class="form-select" required>
                        <option value="low" {{ old('risk_level', $obligation->risk_level) === 'low' ? 'selected' : '' }}>Low</option>
                        <option value="medium" {{ old('risk_level', $obligation->risk_level) === 'medium' ? 'selected' : '' }}>Medium</option>
                        <option value="high" {{ old('risk_level', $obligation->risk_level) === 'high' ? 'selected' : '' }}>High</option>
                        <option value="critical" {{ old('risk_level', $obligation->risk_level) === 'critical' ? 'selected' : '' }}>Critical</option>
                    </select>
            ...
            <div class="grid-2" style="margin-bottom: 1.5rem;">
                <div class="form-group">
                    ...
                    <select name="status" class="form-select" required>
                        @foreach(['active','upcoming','action_required','renewal_in_progress','pending_approval','purchase_in_progress','renewed','expired','cancelled','not_required','archived'] as $status)
                            <option value="{{ $status }}" {{ old('status', $obligation->status) === $status ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="currency" class="form-input" value="{{ old('currency', $obligation->currency) }}" maxlength="3">
            ...
            <div class="grid-2" style="margin-bottom: 1.5rem;">
                <div class="form-group">
                    ...
                    <input type="number" name="estimated_cost" class="form-input" value="{{ old('estimated_cost', $obligation->estimated_cost) }}" step="0.01" min="0">
                ...
                <div class="form-group">
                    ...
                    <select name="recurrence_type" class="form-select">
                        ...
                        <option value="monthly" {{ old('recurrence_type', $obligation->recurrence_type) === 'monthly' ? 'selected' : '' }}>Monthly</option>
                        <option value="quarterly" {{ old('recurrence_type', $obligation->recurrence_type) === 'quarterly' ? 'selected' : '' }}>Quarterly</option>
                        <option value="yearly" {{ old('recurrence_type', $obligation->recurrence_type) === 'yearly' ? 'selected' : '' }}>Yearly</option>
                    </select>
            ...
            <div class="grid-2" style="margin-bottom: 1.5rem;">
                <div class="form-group">
                    ...
                    <input type="number" name="recurrence_interval" class="form-input" value="{{ old('recurrence_interval', $obligation->recurrence_interval) }}" min="1">
                ...
                <div class="form-group" style="display: flex; gap: 1.5rem; align-items: center; padding-top: 2rem;">
                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                        <input type="checkbox" name="renewal_required" value="1" {{ old('renewal_required', $obligation->renewal_required) ? 'checked' : '' }}>
                        <span>Renewal Required</span>
                    </label>
                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                        <input type="checkbox" name="auto_renew" value="1" {{ old('auto_renew', $obligation->auto_renew) ? 'checked' : '' }}>
            ...
            <div class="form-group" style="margin-bottom: 1.5rem;">
                ...
                <textarea name="description" class="form-input" rows="3">{{ old('description', $obligation->description) }}</textarea>
                @error('description') <span style="color: var(--destructive); font-size: 0.875rem;">{{ $message }}</span> @enderror
            </div>

            <div class="form-group" style="margin-bottom: 1.5rem;">
                <label class="form-label">Notes</label>
                <textarea name="notes" class="form-input" rows="3">{{ old('notes', $obligation->notes) }}</textarea>
