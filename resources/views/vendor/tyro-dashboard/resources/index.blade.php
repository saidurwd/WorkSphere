    ...
    <div class="table-container">
        <table class="table">
            ...
            <tbody>
                ...
                <tr style="cursor: pointer;" onclick="window.location='{{ route($dashboardRoute::name('resources.show'), [$resource, $item->id]) }}'">
                    @foreach($config['fields'] as $key => $field)
                    @if(!($field['hide_in_index'] ?? false))
                    <td data-col="{{ $key }}">
                        @if($field['type'] === 'file')
                            @if($item->$key)
                                @php
                                    $fileUrl = Storage::url($item->$key);
                                @endphp
                                <a href="{{ $fileUrl }}" target="_blank" style="color: var(--primary); text-decoration: none;" onclick="event.stopPropagation()" title="Download">
                        ...
                        @elseif($field['type'] === 'markdown')
                            {{ Str::limit(strip_tags($item->$key), 50) }}
                        @elseif($field['type'] === 'datetime-local' && $item->$key)
                            {{ \Illuminate\Support\Carbon::parse($item->$key)->format('d M Y h:i A') }}
                        @elseif($field['type'] === 'date' && $item->$key)
                            {{ \Illuminate\Support\Carbon::parse($item->$key)->format('d M Y') }}
                        @else
                    ...
                    <td style="text-align: right;" onclick="event.stopPropagation()">
                        <div class="table-actions" style="justify-content: flex-end;">
                            <a href="{{ route($dashboardRoute::name('resources.show'), [$resource, $item->id]) }}" class="btn btn-icon btn-ghost" title="View">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            ...
                            <a href="{{ route('estate-staff.print', $item->id) }}" class="btn btn-icon btn-ghost" title="Print" target="_blank">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            ...
                            @endif
                            @if(!($isReadonly ?? false))
                            <a href="{{ route($dashboardRoute::name('resources.edit'), [$resource, $item->id]) }}" class="btn btn-icon btn-ghost" title="Edit">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            </a>
                            <form action="{{ route($dashboardRoute::name('resources.destroy'), [$resource, $item->id]) }}" method="POST" style="display: inline;" id="delete-resource-item-form-{{ $item->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-icon btn-ghost text-danger" title="Delete" onclick="if (confirm('Are you sure you want to delete this item?')) { document.getElementById('delete-resource-item-form-{{ $item->id }}').submit(); }">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
